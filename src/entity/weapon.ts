...
export default class Weapons {
	static bullet(owner: Player) {
		return new Weapon('bullet', owner, 7, false, () => {
			...
			owner.weapon.max_attack_cooldown = Math.floor(Math.max(20, 40 - owner.intelligence * 0.3));
	...
	static fireball(owner: Player) {
		return new Weapon('fireball', owner, 3, false, () => {
	...
	static arrow(owner: Player) {
		return new Weapon('arrow', owner, 4, false, () => {
